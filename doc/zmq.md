...
The LitecoinII Core daemon can be configured to act as a trusted "border
router", implementing the litecoinII wire protocol and relay, making
...
The ZeroMQ feature in LitecoinII Core requires the ZeroMQ API >= 4.0.0
...
during the *configure* step of building litecoinIId:
...
For instance:
    ...
    $ litecoinIId -zmqpubhashtx=tcp://127.0.0.1:28332 \
               -zmqpubhashtx=tcp://192.168.1.2:28332 \
               -zmqpubrawtx=ipc:///tmp/litecoinIId.tx.raw \
...
These options can also be provided in litecoinII.conf.
...
From the perspective of litecoinIId, the ZeroMQ socket is write-only; PUB
sockets don't even have a read function. Thus, there is no state
introduced into litecoinIId directly. Furthermore, no information is
...
using. LitecoinIId appends an up-counting sequence number to each
