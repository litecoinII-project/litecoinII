    ...
    const bool sign)
{
    ...
    while (true) {
        ...
        // the recipients are funded through the pegout kernel instead of traditional LC2 outputs.
        ...
        // Calculate transaction size and the total necessary fee amount (includes LC2 and MWEB fees)
    ...
    if (new_tx.mweb_type != MWEB::TxType::LC2_TO_LC2) {
...
bool TxAssembler::AttemptCoinSelection(InProcessTx& new_tx, const CAmount& nTargetValue) const
{
    ...
    if (new_tx.recipients.front().IsMWEB()) {
        ...
        if (SelectCoins(new_tx, nTargetValue, params_pegin)) {
            return std::any_of(new_tx.selected_coins.cbegin(), new_tx.selected_coins.cend(), is_lc2);
        }
    } else {
        // First try to construct a LC2-to-LC2 transaction
        CoinSelectionParams mweb_to_mweb = new_tx.coin_selection_params;
        mweb_to_mweb.input_preference = InputPreference::LC2_ONLY;
        ...
        // If LC2-to-LC2 fails, create a peg-out transaction
